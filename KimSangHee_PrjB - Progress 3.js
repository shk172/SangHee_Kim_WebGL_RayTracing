...
//
	...
	var hitPoint = vec3.fromValues(inRay.orig[0] + t0*inRay.dir[0], 
								inRay.orig[1] + t0*inRay.dir[1],
								inRay.orig[2] + t0*inRay.dir[2]);
...
//      otherwise ray hit BETWEEN the lines.
	if((hitPoint[0] - Math.floor(hitPoint[0]/this.xgap)) < this.lineWidth || (hitPoint[1] - Math.floor(hitPoint[1]/this.ygap)) < this.lineWidth){
...
// --lamp[0] is a point-light source at location (5,5,5).
	...
	this.grid = new CGeom(JT_GNDPLANE);
...
// Create an image by Ray-tracing.   (called when you press 'T' or 't')
	...
	var hit = 0;
  	...
  	for(var j=0; j< this.imageBuffer.ySiz; j++) {					// for the j-th row of pixels
  		for(var i=0; i< this.imageBuffer.xSiz; i++) {					// and the i-th pixel on that row,
			...
			this.eyeRay = this.camera.setEyeRay(this.eyeRay,i+0.5,j+0.5);						  // create ray for pixel (i,j)
			hit = this.grid.traceGrid(this.eyeRay);						// trace ray to the grid
			if(hit==0) {
				vec4.copy(colr, this.grid.gapColor);
			}
			else if (hit==1) {
				vec4.copy(colr, this.grid.lineColor);
			}
  ...
  // Set the texture unit 0 to be driven by the sampler
	...
	console.log(this.imageBuffer.iBuf);
}
CScene.prototype.increaseZ = function(){
	this.grid.zGrid += 0.5;
	main();
}

CScene.prototype.DecreaseZ = function(){
	this.grid.zGrid -= 0.5;
	main();
}
...
function draw(gl, scene, vertices, u_ViewMatrix, viewMatrix, u_ProjMatrix, projMatrix){
	...
	gl.drawArrays(gl.LINE_STRIP, 0, vertices/4); 
 	//------------------------------------------
  ...
  //------------------------------------------
	...
	gl.drawArrays(gl.TRIANGLE_STRIP, 0, vertices/4); 
